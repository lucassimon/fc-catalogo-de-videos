...
def test_category_validator_with_data():
    ...
    is_valid = validator.check(serializer_class=serializer_class, data=data)
...
def test_category_validator_with_none_data():
    validator = CategoryValidator()
    serializer_class = CategorySerializer
    is_valid = validator.check(serializer_class=serializer_class, data=None)
    assert is_valid is False
...
def test_category_validator_with_empty_data():
    validator = CategoryValidator()
    serializer_class = CategorySerializer
    is_valid = validator.check(serializer_class=serializer_class, data={})
    assert is_valid is False
    assert validator.errors == {"title": ["This field is required."]}
